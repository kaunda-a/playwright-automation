...
export async function POST(request: Request) {
  try {
    ...
    if (botDuration > 0) {
      setTimeout(async () => {
        await launchedInstance. BehaviorSimulator.stop();
...
async function launchEnhancedInstance(bot: Bot, location?: string): Promise<{
  ...
  try {
    ...
    await context.addInitScript(() => {
      ...
      Object.defineProperty(newNavigator, 'hardwareConcurrency', { get: () => 8 });
      Object.defineProperty(newNavigator, 'deviceMemory', { get: () => 8 });
      Object.defineProperty(newNavigator, 'maxTouchPoints', { get: () => 0 });
    ...
    const behaviorSimulator = new BehaviorSimulator(page, browser, context, {
      mouseMoveInterval: 3000 + Math.random() * 2000,
      scrollInterval: 5000 + Math.random() * 3000,
      scrollAmount: 100 + Math.floor(Math.random() * 50),
      mouseMoveSteps: 10 + Math.floor(Math.random() * 5),
    }, true); // Pass true for isEnhancedInstance
  
    await behaviorSimulator.start();
